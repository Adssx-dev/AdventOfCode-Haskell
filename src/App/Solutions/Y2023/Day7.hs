...
part1 inputStr = Just $ sum $ zipWith (\x y -> x * bid y) [1..] $ sortBy (compareHands compare) hands
    where
        hands = map (loadHand calculateHandType) $ lines inputStr

part2 inputStr = Just $ sum $ zipWith (\x y -> x * bid y) [1..] $  sortBy (compareHands compareDeckPt2) hands
    where
        hands = map (loadHandPt2 calculateHandTypePt2) $ lines inputStr
