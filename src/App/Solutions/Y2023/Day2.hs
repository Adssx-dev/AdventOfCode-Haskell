...
part1 inputStr = Just $ sum $ map fst possibleGames
    where
        games = map loadGame $ lines inputStr
        possibleGames = filter (isGamePossible [12, 13, 14]) games

part2 :: [Char] -> Maybe Int
part2 inputStr = Just $ sum $ map calculateGamePower games 
    where
        games = map loadGame $ lines inputStr
        possibleGames = filter (isGamePossible [12, 13, 14]) games
